...
Input:char **pName
	,char **pValue
	int TokenNum
Output: char *MsgBuf	//json token string
Return: length of string
*/
int osPackageJsonStr(char **pName,char **pValue,int TokenNum
							,char *MsgBuf)
{
	int i=0;
	int len=0;
...
/*
char MsgBuf[]="{\"action\":(\"set_video\",}\"width\":\"400\",\"height\":\"300\",\"brightness\":\"50\",\"saturate\":\"55\"}";
//char *MsgBuf="action:set_video,width:400,height:300,brightness:50";	//Do NOT define like this,will lead to Segment fault!
Input: char *MsgBuf	//json token string
Output:char **pName,char **pValue	//pairs of name:value 
Return: token number

error to parse {"action":"start_live","RtspUrl":"rtsp://192.168.2.10:10554/44d1.mp4"}
*/
int osJsonParseMsg(char *MsgBuf
				,char **pName,char **pValue)
{
	int ret=0;
	char *pToken[MAX_JSON_TOKEN]={NULL};
	char *pTemp1=NULL,*pTemp2=NULL;
//	char *pName[MAX_JSON_TOKEN]={NULL};
//	char *pValue[MAX_JSON_TOKEN]={NULL};
//	char Cut1[] = {',','\n','\0'};
//	char Cut2[] = {':','\n','\0'};
	int i=0;

	if (!(pName&&pValue&&MsgBuf))
		return -1;

	pToken[0]=strtok(MsgBuf, ",");
	
	for (i=1; i<MAX_JSON_TOKEN; i++)
	{
	...
	for (i=0; i<MAX_JSON_TOKEN; i++)
	{
		if (NULL==pToken[i])
			break;
		//VosPrint("Json Token[%d]=%s\n",i,pToken[i]);
		
		pTemp1 = strtok(pToken[i], ":");
		//pTemp2 = strtok(NULL, ":");	//will cut string
		pTemp2 = strtok(NULL, "\0");
		//pTemp2=pTemp1+strlen(pTemp1)+1;	//cause segment fault!

		if ((!pTemp1)||(!pTemp2))
			break;
		
	//	VosPrint("Json NameSrc[%d]=%s\n",i,pTemp1);
	//	VosPrint("Json ValueSrc[%d]=%s\n",i,pTemp2);

		while(*pTemp1 != '\"')
			pTemp1++;
		pName[i] = strtok(pTemp1, "\"");
	//	VosPrint("Json Name[%d]=%s\n",i,pName[i]);
		while(*pTemp2 != '\"')
			pTemp2++;		
		pValue[i] = strtok(pTemp2, "\"");		
	//	VosPrint("Json Value[%d]=%s\n",i,pValue[i]);		
	}
	
	ret=i;		//return token number
